...
1. **Assemble the Input Inventory & Finalize Ledger**:
   ...
   - Read `/session/plan/minor_issues.md`: the accumulated non-blocking debt checklist (may be missing when no minor issues were ever recorded).
   - Read `/session/plan/base_commit.txt` (if exists) and inspect cumulative changes via `git diff $(cat /session/plan/base_commit.txt)..HEAD`.
   - Read `/session/final_review.md` **if verdict was FIX**: address all defects and remediation directives specified by `final-reviewer`.
   - Read `/session/final_decision.txt` **only if it exists**: it carries the user's rejection feedback from a prior `final_approval` round; address every point it raises.
   - Read `/session/final_fallback_decision.txt` **only if it exists**: it carries human feedback and guidance when recovering from an exhausted final review loop fallback (`final_fallback`); address every point it raises.

2. **Respect Strict Scope Guardrails (No Repo-Wide Refactoring)**:
   - You may ONLY modify:
     1. Files referenced by open items in `/session/plan/minor_issues.md`;
     ...
     4. Files implicated by `/session/final_review.md`, `/session/final_decision.txt`, or `/session/final_fallback_decision.txt` feedback.
...
3. **Clean Up Minor Issues & Review Feedback**:
   - Work through `/session/plan/minor_issues.md` item by item; tick each resolved item `- [ ]` → `- [x]` as you complete it.
   - If remediation was requested in `/session/final_review.md`, implement all required fixes within scope.
...
4. **Backstop Known-Broken Steps**:
   ...
   - If fixed and verified, update its status in `/session/plan/todo.yaml` to `completed`; otherwise leave the `skipped (known-broken)` marker intact and report it.
...
6. **Generate the Cleanup Summary**:
   - Write a summary of cleanup work to `/session/plan/cleaner_summary.md`:
     ...
     - How `/session/final_review.md`, `/session/final_decision.txt`, or `/session/final_fallback_decision.txt` feedback was addressed (when applicable)
