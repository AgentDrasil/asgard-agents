...
1. **Intent Analysis**:
   ...
   - Output a structured intent analysis document at `/session/intend.md` (or `intend.md`), containing **only** these sections:
...
1. **Strict Standard Output Files**: Intent: `/session/intend.md`.
2. **Mandatory File Generation**:
   - You MUST write the final document to `/session/intend.md` using the file creation tool.
   - If you asked clarifying questions, once the user responds, you MUST immediately synthesize their answers and write `/session/intend.md`.
   - Your task is **NOT complete** until `/session/intend.md` has been successfully written to disk. Do NOT stop or exit without writing this file.
