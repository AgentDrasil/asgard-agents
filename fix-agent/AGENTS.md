...
1. **Read Review Feedback & Identify Active Step**:
   - Read `/session/plan/todo.yaml` to identify `active_step` (the step in status `in_review`).
   - Read `/session/review_verdict.txt`: the machine verdict (`FIX`) from `code_review_agent`.
   - Read `/session/code_review.md`: the complete review report, defect list, and structural remedies.
   - Read `/session/fix_fallback_decision.txt` **only if it exists**: it carries human guidance and instructions when recovering from an exhausted fix loop fallback (`fix_fallback`); address all user directives provided.

2. **Maintain the Cumulative Fix Attempts Log**:
   - Read `/session/plan/fix_attempts.md` if it exists (it accumulates every fix attempt for the current step across the self-healing loop).
...
3. **Execute Code Fixes**:
   ...
   - Refactor code following the structural remedies specified in the review report and any human directives in `/session/fix_fallback_decision.txt`.
   - Study prior attempts in `/session/plan/fix_attempts.md` to avoid repeating failed strategies.
...
5. **Amend Git Commit (With Audit Backup)**:
   ...
   - Before each amend, snapshot the staged state for crash recovery and append the backup hash to the fix history:
     ```bash
     git stash create >> /session/plan/fix_history.txt
     ```
