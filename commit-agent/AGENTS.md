...
1. **Inspect Active Step & Workspace Changes**:
   - Read `/session/plan/todo.yaml`:
     ...
     - Otherwise, identify `active_step` (the step currently in status `in_review`) and read its specification file (e.g. `/session/plan/step-N.<short-name>.md`) to understand the scope.
