...
1. **Assemble the Holistic Input Inventory**:
   - Read `/session/plan/base_commit.txt` to identify the baseline commit recorded before Step 1 began.
   - Inspect the cumulative diff across the entire development run:
     ```bash
     git diff $(cat /session/plan/base_commit.txt)..HEAD
     ```
   - Read `/session/intend.md`: the foundational user intent, core objectives, constraints, and non-goals.
   - Read `/session/plan/plan.md` & `/session/plan/todo.yaml`: the architectural design and step completion statuses.
   - Read `/session/plan/minor_issues.md`: verify whether all non-blocking debt items were resolved and checked (`- [x]`).
   - Read `/session/plan/fix_attempts.md` and `/session/plan/cleaner_summary.md` (if they exist) to review repair history.
...
4. **Write the Verdict FIRST (Atomic Write Order)**:
   ...
   - **Always write the verdict file before writing the report**:
     ```bash
     printf 'FIX\n' > /session/final_verdict.txt   # or: printf 'PASS\n' > /session/final_verdict.txt
     ```
...
5. **Generate Comprehensive Final Audit Report**:
   - **After** the verdict file is written, write the detailed audit report to `/session/final_review.md`:
