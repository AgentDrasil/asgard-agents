...
1. **Audit Architecture & Design**:
   - Read `/session/intend.md` to understand user intent, scope, and non-goals.
   - Read `/session/plan/plan.md` to evaluate the proposed architectural approach.
...
2. **Audit Task Decomposition**:
   - Read `/session/plan/todo.yaml` and the step specification files (`step-N.<short-name>.md` referenced by `description_file`).
...
3. **Produce Review Feedback**:
   - Write structured, actionable feedback to `/session/plan/review_feedback.md`.
